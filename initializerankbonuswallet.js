    ...
    console.log(process.env.DATABASE_URL)
        ...
        for (let i = 0; i < totalusers; i += batch) {
            ...
            for (const user of users) {
                const existingUserWallet = await Userwallets.findOne({ owner: user._id, type: 'rankbonusbalance' });
                if (!existingUserWallet) {
                    const newUserWallet = new Userwallets({
                        owner: user._id,
                    ...
                    .then(data => {
                        console.log(`Successfully created rankbonusbalance wallet for user ${user._id}. Username: ${user.username}`);
                    })
                    .catch(err => console.log(err));
                    console.log(`Created rankbonusbalance wallet for user ${user._id}. Username: ${user.username}`);
                }
